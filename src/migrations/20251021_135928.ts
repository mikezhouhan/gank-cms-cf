...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`DROP INDEX \`blogs_slug_idx\`;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`locale\` text DEFAULT 'en' NOT NULL;`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`author_image_id\` integer REFERENCES media(id);`)
  await db.run(sql`ALTER TABLE \`blogs\` ADD \`author_link\` text;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_blogs\`("id", "title", "slug", "content", "excerpt", "featured_image_id", "author_id", "status", "published_at", "updated_at", "created_at") SELECT "id", "title", "slug", "content", "excerpt", "featured_image_id", "author_id", "status", "published_at", "updated_at", "created_at" FROM \`blogs\`;`)
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_payload_locked_documents_rels\`("id", "order", "parent_id", "path", "users_id", "media_id", "blogs_id") SELECT "id", "order", "parent_id", "path", "users_id", "media_id", "blogs_id" FROM \`payload_locked_documents_rels\`;`)
